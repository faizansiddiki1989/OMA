...
/********************************
 * File Name : FOPS_CS_Restuarant_Validation
...
*/
 ...
 define(["N/https", "N/ui/dialog"], function (
  https, dialog) {
  ...
  var fields = [
    ...
    {'id':'custrecord_rest_cust','label':'Customer'},
    {'id':'custrecord_rest_tax_pay_id','label':'Tax Payer Id Number'},
    {'id':'custrecord_rest_tax_pay_id_num','label':'Confirm Tax Payer Id Number'},
    {'id':'name','label':'Business Name'},
    {'id':'custrecord_rest_add_line_1','label':'Address Line1'},
    {'id':'custrecord_rest_tx_city','label':'City'},
    {'id':'custrecord_rest_tax_sate','label':'State'},
    {'id':'custrecord_rest_zip_code_tax','label':'Zip'},

    {'id':'custrecord_rest_cont_tax_person','label':'Contact Person'},
    {'id':'custrecord_rest_tax_tele','label':'Contact Telephone'},
    {'id':'custrecord_rest_corp_name','label':'Corporate Name'},
    {'id':'custrecord_rest_mail_address_line1','label':'Mailing Address Line 1'},
    //{'id':'custrecord_rest_mail_address_line2','label':'Mailing Address Line 2'},
    
    {'id':'custrecord_mail_rest_state','label':'Mailing State'},
    {'id':'custrecord_mail_rest_city','label':'Mailing City'},
    {'id':'custrecord_mail_rest_zip','label':'Mailing Zip'},
    {'id':'custrecord_mail_rest_corpo_cont','label':'Mailing Corporate Contact'},
    {'id':'custrecord_mail_cont_rest','label':'Mailing Contact Telephone'},
    {'id':'custrecord_busines_open_rest_date','label':'Business Open Date'},
    {'id':'custrecord_payment_rest_period','label':'Payment Period'}
   ];
 ...
 // pageInit
  function pageInit(context){
    debugger;
    jQuery("#custrecord_rest_tax_pay_id,#custrecord_rest_tax_pay_id_num").keyup(function(e){
        taxNumberFormat(e);      
        });

    jQuery("#custrecord_rest_tax_tele,#custrecord_mail_cont_rest").keyup(function(e){
           telePhoneNumberFormat(e)
        });

    openDateinit = context.currentRecord.getValue({fieldId:'custrecord_busines_open_rest_date'});

    var mandatoryTag = '<label class="uir-required-icon">*</label>';
    jQuery("#custrecord_mail_rest_city_fs_lbl").append(mandatoryTag);
    jQuery("#custrecord_mail_rest_state_fs_lbl").append(mandatoryTag);
    jQuery("#custrecord_mail_rest_zip_fs_lbl").append(mandatoryTag);
    jQuery("#custrecord_rest_mail_address_line1_fs_lbl").append(mandatoryTag);
 ...
 //validateField
  function validateField(context){
    var currentRecord = context.currentRecord;
     var option = {
        ...
        if (context.fieldId == "custrecord_rest_tax_pay_id") {
         var taxnumber = currentRecord.getValue({fieldId:"custrecord_rest_tax_pay_id"});
          if(taxnumber != ''){
          ...
          alert('validateField');      currentRecord.setValue({fieldId:"custrecord_rest_tax_pay_id",value:finalStr,ignoreFieldChange:true});
  ...
  function fieldChanged(context) {
    ...
    try {
        ...
        else if (context.fieldId == "custrecord_rest_tax_pay_id_num") {
          confirmTax(currentRecord,option)
        } else if (context.fieldId == "name") {
          nameName(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_add_line_1") {
          address1(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_add_line2") {
          address2(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_tx_city") {
        ...
        else if (context.fieldId == "custrecord_rest_zip_code_tax") {
          zipcode(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_cont_tax_person") {
          taxPerson(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_tax_tele") {
          telephoneNumber(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_corp_name") {
           corporateName(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_mail_address_line1") {
          malingAddress1(currentRecord,option)
        } else if (context.fieldId == "custrecord_rest_mail_address_line2") {
          malingAddress2(currentRecord,option)
        } else if (context.fieldId == "custrecord_mail_rest_city") {
          malingCity(currentRecord,option)
        } else if (context.fieldId == "custrecord_mail_rest_zip") {
          malingZip(currentRecord,option)
        } else if (context.fieldId == "custrecord_mail_rest_corpo_cont") {
          malingCorporateName(currentRecord,option)
        } else if (context.fieldId == "custrecord_mail_cont_rest") {
           malingTelephone(currentRecord,option)
        }else if (context.fieldId == "custrecord_busines_open_rest_date") {
          var  changeDate = currentRecord.getValue({fieldId:'custrecord_busines_open_rest_date'});
          if(changeDate != openDateinit){
            falg = true
          }
        }else if (context.fieldId == "custrecord_rest_sameas_above") {
          // same as above
          var sameAsAbove = currentRecord.getValue({
            fieldId: "custrecord_rest_sameas_above",
          });
          if (sameAsAbove) {
            currentRecord.setValue({
              fieldId: "custrecord_rest_corp_name",
            ...
            currentRecord.setValue({
              fieldId: "custrecord_rest_mail_address_line1",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_add_line_1",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_rest_mail_address_line2",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_add_line2",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_mail_rest_state",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_tax_sate",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_mail_rest_city",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_tx_city",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_mail_rest_zip",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_zip_code_tax",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_mail_rest_corpo_cont",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_cont_tax_person",
              }),
              ignoreFieldChange: true
            });
            currentRecord.setValue({
              fieldId: "custrecord_mail_cont_rest",
              value: currentRecord.getValue({
                fieldId: "custrecord_rest_tax_tele",
              }),
              ignoreFieldChange: true
            });
          }
...
/*
    ...
    var taxnumber = currentRecord.getValue({fieldId:"custrecord_rest_tax_pay_id"});
          ...
          if(taxnumber && !taxregex.test(taxnumber)){
            ...
            if(!saveRecord) {
              ...
              dialog.alert(option);
              currentRecord.setValue({
                fieldId: 'custrecord_rest_tax_pay_id',
                value: '',
                ignoreFieldChange: true,
                forceSyncSourcing: false
              });
              document.getElementById('custrecord_rest_tax_pay_id').focus();
  ...
  function confirmTax(currentRecord,option,saveRecord){
    var taxnumber = currentRecord.getValue({fieldId:"custrecord_rest_tax_pay_id"});
    var confTaxnumber = currentRecord.getValue({fieldId:"custrecord_rest_tax_pay_id_num"});
          ...
          if(taxnumber && confTaxnumber && taxnumber == confTaxnumber){
            ...
            if(taxnumber.length !=10 && confTaxnumber.length != 10){
              if(!saveRecord) {dialog.alert(option);
                currentRecord.setValue({
                  fieldId: 'custrecord_rest_tax_pay_id_num',
                  value: '',
                  ignoreFieldChange: true,
                  forceSyncSourcing: false
                });
                document.getElementById('custrecord_rest_tax_pay_id_num').focus();
              }
              if(saveRecord) {
              return obj
              }
  ...
  function telephoneNumber(currentRecord,option,saveRecord){
    ...
    if (businesstelephone && !businesstelephoneregex.test(businesstelephone)) {
        if(!saveRecord){
            ...
            document.getElementById('custrecord_rest_tax_tele').focus();
      ...
      if(!saveRecord) 
      {
        dialog.alert(option);
         currentRecord.setValue({
            ...
            document.getElementById('custrecord_mail_cont_rest').focus();
      }
  ...
  function BusinessOpenDate(currentRecord,option,saveRecord){
    var openDate = currentRecord.getValue({
      fieldId: "custrecord_busines_open_rest_date",
    });
  ...
  function BusinessPaymentPeriod(currentRecord,option,saveRecord){
    var period = currentRecord.getValue({
      fieldId: "custrecord_payment_rest_period",
    });
 ...
 function paymentsInDb(id){
  ...
  var filters = [
      ["type","anyof","CustPymt"], 
      "AND", 
      ["custbody_rest_tax_bus_id","anyof",id],
      "AND", 
      ["mainline","is","T"]
    ];
  ...
  if(searchPayment){
    searchPayment.forEach(function(result){
      ...
      var qtrYear =  result.custbody_hotel_year_tax.value;
      ...
      var monYear1 = result.custbody_tax_year.value;
      var monYear2 = result.custbody_tele_mnthly_year_2.value;
      var monYear3 = result.custbody_tele_mnthly_year3.value;
      var obj = {
        "1": ['1','2','3'],
        "2": ['4','5','6'],
        "3": ['7','8','9'],
