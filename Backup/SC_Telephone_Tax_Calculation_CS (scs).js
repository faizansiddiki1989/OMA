...
define(["N/record", "N/search", "N/runtime", "N/ui/dialog", "N/format"], function(
    ...
    function saveRecord(context) {
        ...
        try {
            ...
            var currentRecord = context.currentRecord;
            var bId = currentRecord.getValue({
                fieldId: "custbody_business_id",
            });
          	log.debug("Save Record Business ID", bId);
            if (bId) {
                ...
                if (!paymentType) {
                    ...
                    if (!context.currentRecord.id || (context.currentRecord.id && yymmChange)) {
                        ...
                        if (yearinDb) {
                            ...
                            if (paymentMonths) {
                    			log.debug("paymentMonths", paymentMonths);
                                alert(
    ...
    function paymentsInDb(id) {
        ...
        searchPayment.forEach(function(result) {
            ...
            if (quarter && qtrYear && (qtrAmt1 || qtrAmt2 || qtrAmt3)) {
                ...
                hasData = true;
                var quart = result.getValue("custbody_quarterly");
        ...
        log.debug("Payment Validation", payment.validation);
