...
define(['N/search', 'N/record', 'N/runtime'], function (search, record, runtime) {
  ...
  function getSubCustomersList(custid, countType) {
    ...
    customerSearchObj.run().each(function (result) {
      ...
      arrCustomerIds.push(subCustomerid);
    ...
    var parking_application = search.create({
      ...
      filters:
        [
          ["custrecordscs_customers_name", "anyof", arrCustomerIds],
          "AND",
          ["isinactive", "is", "F"]
        ],
      columns:
        [
          ...
          search.createColumn({ name: "custrecordscs_customers_name", label: "Customer ID" }),
        ]
    });
    log.debug('parking_application', parking_application.runPaged().count)
    // Add additional code 

    //GET PARKING RECORD IDS
    var parkingid = '';
    var tempCustId = '';
    parking_application.run().each(function (parking) {
      // .run().each has a limit of 4,000 results
      parkingid = parking.getValue('internalid');
      tempCustId = parking.getValue('custrecordscs_customers_name');
      parkingArrayIdS[tempCustId] = parkingid;
      return true;
    });

    //MERGE PARKING ID WITH MAIN DATA
    for (var i = 0; i < data.length; i++) {
      var internalid = data[i].internalid;
      if (parkingArrayIdS[internalid]) {
        data[i].parkingid = parkingArrayIdS[internalid];
      }
    }
