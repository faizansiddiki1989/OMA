...
define(['N/record', 'N/runtime', 'N/search', 'N/ui/serverWidget'], function(record, runtime, search, serverWidget) {
    ...
    const CLIENTSCRIPT_FILEID = 45962; //FOPS_CS_ViewMode.js
        ...
        try {
            ...
            if (context.type == context.UserEventType.VIEW) {
                ...
                if(recordForm.customform[0].value == PAYMENTFORMS.RESTAURANT 
                    ...
                    currentForm.clientScriptFileId = CLIENTSCRIPT_FILEID; //FOPS_CS_ViewMode.js
                
                log.debug('formcustom', recordForm.customform[0].value);
                if (recordForm.customform[0].value == PAYMENTFORMS.RESTAURANT 
                    || recordForm.customform[0].value == PAYMENTFORMS.TOBACCO
                    || recordForm.customform[0].value == PAYMENTFORMS.HOTEL
                    || recordForm.customform[0].value == PAYMENTFORMS.TELEPHONE) {
    ...
    function afterSubmit(context) {
        ...
        //IF FORM IS NOT FOPS FORM THEN RETURN
        if(currentForm != PAYMENTFORMS.RESTAURANT && 
            currentForm != PAYMENTFORMS.HOTEL && 
            currentForm != PAYMENTFORMS.TOBACCO &&
            currentForm != PAYMENTFORMS.TELEPHONE)
            return;
